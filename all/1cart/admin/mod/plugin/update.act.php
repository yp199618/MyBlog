...
if($do=='install'){
	...
	elseif(file_exists($install_dir_2)){
		...
		echo js_confirm("安装文件不存在，需要下载插件，自动下载可能会覆盖二次开发过的文件，选确认将自动下载插件并覆盖安装，取消返回",u('plugin','down',array('code'=>$code)),'history.go(-1)');
	}
}
elseif($do=='uninstall'){
	if(file_exists($install_dir_1)){
	...
	if($_GET['over']==1){
		...
		echo '<script language="javascript" type="text/javascript">if (confirm("更新插件需要下载，自动下载可能会覆盖二次开发过的文件，选确认将自动下载插件并覆盖安装，取消返回")){location.href="'.u('plugin','down',array('code'=>$code,'update'=>1)).'";}else{history.go(-1);};</script>';
	}
}
?>