...
<form action="index.php?mod=<?=MOD?>&act=<?=ACT?>" method="post" name="form1">
  ...
  <? }else{?>
    ...
    您已经可以使用在线抓取阿里妈妈订单的功能，<a href="<?=u(MOD,ACT,array('test'=>1))?>">点击测试</a>（注意，测试代码默认抓取你今天的订单）
    <?php }else{?>
    请设置淘宝联盟密码，完成保存。
    <?php }?>
    </div>
  ...
  <table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
    <tr>
      ...
      <td>&nbsp;<input type="text" name="username" id="username" value="<?=$alimama_data['username']?>" />  <?php if($yun_alimama['buy']==1){?>&nbsp;应用授权时间【<?=date('Y-m-d H:i:s',$a['r']['addtime'])?> 到 <?=date('Y-m-d H:i:s',$a['r']['endtime'])?>】（表示应用插件的有效时间）<?php }?></td>
    </tr>
    ...
    <tr>
      <td align="right">&nbsp;</td>
      <td>&nbsp;<input type="submit" name="sub" value=" 保 存 设 置 " /> &nbsp;保存后更新网站缓存（后台右上角）</td>
    </tr>
  </table>
</form>
...
<form method="get" action="../index.php">
  <table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
    ...
    <tr>
      <td align="right">&nbsp;</td>
      <td>&nbsp;<input type="submit" class="sub" name="daoru" value="获取交易记录" /> 根据时间人工获取订单</td>
    </tr>
  </table>
